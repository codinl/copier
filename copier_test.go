...
func TestCopySameStructWithPointerField(t *testing.T) {
	...
	user := &User{Birthday: &currentTime, Name: "Jinzhu", Nickname: "codinl", Age: 18, FakeAge: &fakeAge, Role: "Admin", Notes: []string{"hello world", "welcome"}, flags: []byte{'x'}}
	newUser := &User{}
	copier.Copy(newUser, user, true)
...
func TestCopyStruct(t *testing.T) {
	...
	employee3 := Employee{}
	ptrToUser := &user
	copier.Copy(&employee3, &ptrToUser, true)
	checkEmployee(employee3, user, t, "Copy From Double Ptr To Ptr")

	employee4 := &Employee{}
	copier.Copy(&employee4, user, true)
...
func TestCopyFromStructToSlice(t *testing.T) {
	...
	if err := copier.Copy(employees, &user, true); err != nil && len(employees) != 0 {
		t.Errorf("Copy to unaddressable value should get error")
	}

	if copier.Copy(&employees, &user, true); len(employees) != 1 {
	...
	employees2 := &[]Employee{}
	if copier.Copy(&employees2, user, true); len(*employees2) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
	...
	employees3 := []*Employee{}
	if copier.Copy(&employees3, user, true); len(employees3) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if copier.Copy(&employees4, user, true); len(*employees4) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
...
func TestCopyFromSliceToSlice(t *testing.T) {
	...
	if copier.Copy(&employees, users, true); len(employees) != 2 {
	...
	employees2 := &[]Employee{}
	if copier.Copy(&employees2, &users, true); len(*employees2) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees3 := []*Employee{}
	if copier.Copy(&employees3, users, true); len(employees3) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if copier.Copy(&employees4, users, true); len(*employees4) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
...
func TestCopyFieldsWithSameNameButDifferentTypes(t *testing.T) {
	...
	err := copier.Copy(obj2, &obj1, true)
	if err != nil {
